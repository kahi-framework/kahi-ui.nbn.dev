...
To support customization via properties, `<Overlay captive>` was removed in favor of composable `<Backdrop>` (non-dismissible) and `<Overlay.Backdrop>` (dismissible).
...
<Button for="overlay-captive-component">
    Open Overlay
</Button>

<Overlay.Container
    logic_id="overlay-captive-component"
>
    <Overlay.Backdrop />
...
Due to the new `<Transition variation="explicit">` implementation, the current variables would have to get extremely verbose. And ultimately seems a very limited use-case to have animations customizable on that level.
