...
To clarify intent "dark" and "light" of theme modes from Component palettes (`dark`, `alert`, `negative`, etc...), `data-palette` is now `data-mode`.
...
### `<DayStepper step>` / `<MonthStepper step>` / `<YearStepper step>` Renamed in Favor of `<* steps>` Property

To align `<Pagination steps>` for consistency, and because `steps` reads better if your stepping is greater than one (1), the `step` property was renamed to `steps`.
...
### `<TextInput characters lines>` Removed in Favor of `<TextInput span_x span_y>`

To be consistent with later additions to the Component APIs, both the `characters` and `lines` properties were renamed to `span_x` and `span_y` properties respectively. With otherwise no additional changes.
...
### `<TimePicker disabled highlight>` Properties Now Accept `string[]` Instead of `string`

Both `disabled` and `highlight` properties were updated to accept string arrays instead of singular strings. Allowing for multiple timestamps to be used.
...
<TimePicker disabled="13:30:15" highlight="15:00:00" />
```
