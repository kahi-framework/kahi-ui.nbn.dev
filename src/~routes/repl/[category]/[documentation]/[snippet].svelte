<script context="module" lang="ts">
    ...
    export const load: Load = async ({fetch, page}) => {
        ...
        const response = await fetch(`/api/v1/documentation/${category}/${documentation}.json`);
        if (!response.ok) {
            return {
                status: 404,
                error: "InvalidDocumentationSnippet",
        ...
        const {snippets} = data.data.properties;
